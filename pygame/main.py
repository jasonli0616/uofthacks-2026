...
from particle_beam import ParticleBeam  # Import the new ParticleBeam class
import random
import os
...
def main(config):
    ...
    # Initialize lives and load heart image
    lives = 5
    heart_image = pygame.image.load(os.path.join(os.path.dirname(__file__), "imgs", "heart.png")).convert_alpha()
    heart_image = pygame.transform.scale(heart_image, (30, 30))  # Scale heart size
    
    # Define timing line and collision box
    TIMING_LINE_X = 250
    COLLISION_BOX = pygame.Rect(TIMING_LINE_X + 50, 0, 40, SCREEN_HEIGHT)  # Wider box for error tolerance
    ...
    while running:
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                if played_string:
                    # Check for enemies in collision box matching the string
                    hit_enemies = [enemy for enemy in enemies_group if enemy.string == played_string and enemy.rect.colliderect(COLLISION_BOX)]
                    if hit_enemies:
                        # Shoot particle beam and remove hit enemies
                        beam = ParticleBeam(player.rect.midright, played_string)
                        beam_group.add(beam)
        ...
        # Draw the timing line
        pygame.draw.line(screen, (255, 0, 0), (TIMING_LINE_X, 0), (TIMING_LINE_X, SCREEN_HEIGHT), 5)
        ...
        # Check for enemies that passed the collision box without being played
        for enemy in enemies_group:
            if enemy.rect.left < COLLISION_BOX.left:
                lives -= 1
                enemy.kill()
                if lives <= 0:
        ...
        for i in range(lives):
            screen.blit(heart_image, (window_size[0] - (i + 1) * 35, 10))
